...
import { generateDockerComposeFile, getNetworkPath } from "../utils/network.utils";
  ...
  downNetwork: (networkId: string) => {
    ...
    return new Promise<string>((resolve, reject) => {
      exec(
        ...
        (error, stdout, stderr) => {
          if (error) {
            logger.error(`Error bringing network down: ${stderr}`, { networkId });
            reject(`Error: ${stderr}`);
          } else {
  ...
  getNetworkStatus: (networkId: string) => {
    ...
    return new Promise<string>((resolve, reject) => {
      exec(
        ...
        (error, stdout, stderr) => {
          if (error) {
            logger.error(`Error fetching network status: ${stderr}`, { networkId });
            reject(`Error: ${stderr}`);
          } else {
            ...
            logger.info("Network status retrieved successfully", { networkId, status });
